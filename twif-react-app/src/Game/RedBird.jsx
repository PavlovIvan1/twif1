...
// const RedBird = ({ currentRound, rocketPosition, onCollision }) => {
//   const maxCrowsPerRound = Math.floor(Math.random() * (5 - 3 + 1)) + 3;
...
//                 width: '60px',
//                 height: '60px',
//                 backgroundImage: `url(${crow.crowFrame === 0 ? '/RedBird.png' : '/RedBird2.png'})`,
//                 backgroundSize: 'contain',
//                 backgroundRepeat: 'no-repeat',
//                 left: `${crow.left}px`,
...
const RedBird = ({ currentRound, rocketPosition, onCollision }) => {
  ...
  useEffect(() => {
    ...
    for (let i = 0; i < maxCrowsPerRound; i++) {
      newCrows.push({
        left: currentLeft,
        bottom: getRandomBottom(),
        isVisible: true,
      ...
      currentLeft += 7000; // Добавляем 4000 пикселей к позиции для следующей вороны
    }
...
function getCloseRandomLeft() {
  const minLeft = 4000; // Установите минимальное расстояние здесь
  const maxLeft = 6000; // Максимальное расстояние
  return Math.random() * (maxLeft - minLeft) + minLeft;
}